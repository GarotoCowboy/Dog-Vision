...
public class TrainerController {
    ...
    public TrainerResponse getById(
            @Parameter(description = "Trainer UUID", required = true)
            @PathVariable UUID id
    ) {
        return service.getById(id);
    }

    @Operation(summary = "Find trainer by registration")
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
                    description = "Trainer found",
    ...
    public TrainerResponse getByRegistration(
            @Parameter(description = "Trainer registration", required = true)
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "204", description = "Trainer deleted successfully", content = @Content),
            @ApiResponse(responseCode = "404", description = "Trainer not found", content = @Content),
            @ApiResponse(responseCode = "401", description = "Unauthenticated", content = @Content)
    })
    @DeleteMapping("/{id}")
